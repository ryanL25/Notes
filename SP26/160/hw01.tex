...
\[
	f'(x) = (x-1)^{3n} + (x-1)^{n} = (x-1)^{n}((x-1)^{2n} + 1)
\]

\[
f'(x) = 0 \rightarrow (x-1)^{n} = 0, ((x-1)^{2n} + 1) = 0
\]

\[
	(x-1)^{n} = 0 \implies x = 1, ((x-1)^{2n}+1) = 0 \text{ has no solution}
\]


\[
f''(x) = 3n(x-1)^{3n-1} + n(x-1)^{n-1}
\]
We see that we need to keep taking derivatives until $ k + 1=n $ such that $ f''(x) \neq 0 $. 

That means that when $ n $ is odd, the point will be a local min, since the $ n $th derivative of $ f(x) =n!$ and $ f''(x) \ge 0 \forall x$. However, when $ n $ is even, then it will be a saddle. 


We also need to check the endpoints $ x=10 $ and $ x=-10 $ 

\[
	f'(10) = 9^{3n} + 9^{n} = 9^{n}(9^{2n}+1), f'(-10) = (-11)^{n}((-11)^{2n}+1) = (-11)^{n}((-11)^{2^{n}}+1) = (-11)^{n}(121^{n} + 1)
\]
When $ x=10 $, $ f'(10) $ is always positive, therefore signifying it is a local max. When $ x=-10 $, $f'(-10) $, it alternates between positive if n is even and negative if n is odd.  
\vspace{5mm}


Thus if n is even: 

$ x = 1 $ is a saddle, $ x=-10 $ is a global min, and $ x=10 $ is a global max. 

\vspace{5mm}
If $ n $ is odd: 

$ x=1 $ is a global min, $ x=-10 $ and $ x=10 $ are global max. 
}
\end{document}
